...
\mathcal{N}_z := \bigcap_{g \in G} H_{z}(gz)
\end{equation}
is called the \emph{normal polygon} with respect to the group $G$ and the point $z_0$.
\end{definition}

\begin{theorem}
Let $\mathcal{N}_z$ be a normal polygon with respect to a group $G \le \PSL{\Z}$ and a point $z \in \mathcal{H}$. $\mathcal{N}_z$ is a fundamental domain for the action of $G$ on $\EU$.
\end{theorem}
\begin{proof}
For the proof of this fact we refer to \Lehner{}, p.146ff.\footnote{Chapter IV, section 7}
\end{proof}
