...
However, if $u \conj{v} \notin \R$, then $u$ and $v$ span a non-degenerate parallelogram $P_{u,v} := \setdef{t u + s v}{t,s \in [0,1)} \subseteq \C$. Translation of $P_{u,v}$ by integer multiples of $u$ and $v$ covers every point in $\C$ exactly once and thus the set
\begin{equation*}
...
Now, still for the case $u \conj{v} \notin \R$, we show that the set $O_{u,v} \subseteq (L_{u,v})^2$ must contain an element of minimal $\eucnorm{.}$-norm: Denote by $S_r := \setdef{x \in \C^2}{\eucnorm{x} \le r}$ and let $r > 0$ be sufficiently large such that $S_r \cap O_{u,v}$ is nonempty. Because 
\begin{equation*}
