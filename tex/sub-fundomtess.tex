...
\end{equation}
We call two points $z, w \in \EC$ \emph{equivalent}, if and only if there is a modular transformation $A \in \PSL{\Z}$ with $A z = w$ and we write  $z \sim w$. The equivalence class (\emph{orbit}) of a point $u/v \in \EC$ is written as $[u/v]_\sim \in \EC/\sim$ and consists of all points
\begin{equation*}
\frac{a u + b v}{c u + d v} \in \EC \quad \text{with }{\mat{a}{b}{c}{d} \in \PSL{\Z}}.
\end{equation*}
We now wish to find a system of representatives for $\EC/\sim$, \ie we want to pick exactly one representative $u_0/v_0$ out of each $[u/v]_\sim$. One approach for this could be to fix $u,v \in \C$, not both zero, then to choose from the set
\begin{equation*}
