...
Also the statement of Lemma~\ref{lem_MapConFracToPathInjective}, the one-to-one correspondence between continued fraction representations and $T$-$U$ words of a certain form, has a nice visual interpretation, as we will illustrate in the following example.
...
\item Set $A_2 := U^3 T U^{-7} T$. As above, label all indisks $A_2 U^k \Indisk$ within $\Forddisk_{c_1}$ with $k$ and go from $A_2 \Indisk$ with label 0 sixteen steps to $A_2 U^{16} \Indisk$ with label 16 (Figure~\ref{fig_ConFracPi}, 3rd row) and from there to $A_2 U^{16} T \Indisk$ within $\Forddisk_{c_2}$, $c_2 = \frac{355}{113}$.
...
Considering also the ``intermediate'' indisks (those with labels between 0 and $b_j$), we describe in this way an \emph{indisk-path}, or in other words a chain of successively tangent disks, through the set of all indisks $A\Indisk$, $A \in \PSL{\Z}$. Such a path always starts at $\Indisk$ and ends at some $U^{e_0}T U^{e_1}T \dots U^{e_n}T\Indisk$, adding exactly one symbol $U$, $\inv{U}$ or $T$ to the group when moving from one indisk to the next. Obviously these paths can be considered as a visual representation of $T$-$U$ words of the form (\ref{eqn_ConFracTUWord}). This makes the ambiguity of continued fractions representations immediately apparent: All different indisk-paths ending in a given Ford disk $\Forddisk_r$ give rise to a different continued fraction representation for $r \in \EQ$.
