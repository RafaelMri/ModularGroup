...
\fundef{g_1}{\Words{\Z}}{\FreeGrp{\Syms} / \langle U \rangle}{(b_0,b_2,\dots,b_n)}{U^{e_0}T U^{e_1}\dots T U^{e_n} T \langle U \rangle} \quad \text{with } e_j := (-1)^j b_j.
\end{equation}
...
\begin{lemma}
The maps $g_1$, $g_2$, $g_3$ and $f$, which shall be defined as above, satisfy
\begin{equation*}
...
\item If $\sigma$ is not empty, the rightmost symbol of its reduced form is $T$.
\end{enumerate}
