...
\item[Case $A = 0$:] The g-disk corresponds to a half-plane of $\C$, which is obtained from the left half-plane, \ie $\Re{z} < 0$ (\resp $\le 0$), by translation by $\frac{-D}{2 \abs{C}}$ followed by a rotation around the origin by $\arg(B)$. The point $\infty$ is member of the generalized disk, if and only if it is closed.
