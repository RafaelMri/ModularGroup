...
\begin{remark}
From Definition~\ref{dfn_GenCircle} we see that the point $\infty = 1/0$ lies on the g-circle $M = \smallmat{A}{B}{C}{D}$ if and only if its left upper matrix entry $A$ is zero. This is consistent with stereographic projection, because $A = 0$ exactly when $M$ corresponds to a line on the complex plane. The image of this line under reverse stereographic projection is a circle on the Riemann sphere going through the north-pole, which we have identified with the point $\infty$.
\end{remark}
...
\item[Case $A < 0$:] The open g-disk $M$ corresponds to the set complement (within $\EC$) of the closed g-disk $-M$. Accordingly, the closed g-disk $M$ is the complement of the open g-disk $-M$. In both cases the point $\infty$ is contained in the g-disk.
\end{description}
\end{remark}
