...
\end{equation*}
Next, we substitute $z = u / v$, with $u,v \in \C$, $v \ne 0$ and scale by $\conj{v} \cdot v = \abs{v}^2 > 0$, which yields the equivalent equation
\begin{equation}
...
This definition does not depend on the choice of $u$ and $v$. If $u/v = u^\prime/v^\prime$, \ie $u^\prime = \lambda u$ and $v^\prime = \lambda v$ for some $\lambda \in \C \setminus \{0\}$, then also
\begin{equation*}
...
\begin{remark}
From Definition~\ref{dfn_GenCircle} we see that the point $\infty = 1/0$ lies on the g-circle $M = \smallmat{A}{B}{C}{D}$, if and only if its left upper matrix entry $A$ is zero. This is consistent with stereographic projection, because $A = 0$ is exactly the case when $M$ corresponds to a line on the complex plane. The image of this line under reverse stereographic projection is a circle on the Riemann sphere going through the north-pole, which we have identified with the point $\infty$.
\end{remark}
...
\begin{description}
\item[$A > 0$:] The g-disk corresponds to a disk in the usual sense within $\C$. Its center is given by (\ref{eqn_GenCircCenter}) and its radius by (\ref{eqn_GenCircRadius}).
\item[$A = 0$:] The g-disk corresponds to a halfplane of $\C$, which is obtained from the left halfplane, $\Re{z} < 0$ (\resp $\le 0$), by translation by $\frac{-D}{2 \abs{C}}$ and rotation by $-\arg(C)$. The point $\infty$ is member of the generalized disk, if and only if it is closed.
\item[$A < 0$:] The open g-disk $M$ corresponds to the set complement (within $\EC$) of the closed g-disk $-M$. Accordingly, the closed g-disk $M$ is the complement of the open g-disk $-M$. In particular, $\infty$ is a member of the g-disk.
\end{description}
\end{remark}

It is now easy to show that generalized circles and generalized disks are preserved under Möbius transformations.

\begin{theorem}
\label{thm_MoebiusGenCircle}
Let $M \in \Mat{\C}{2}{2}$ be a Hermitian matrix with $\det(M) < 0$ and $P \in \GL{\C}$. The image of the g-circle (\resp open/closed g-disk) $M$ under the Möbius transformation $\phi$ corresponding to the matrix $P \in \GL{\C}$ is the g-circle (\resp open/closed g-disk)
\begin{equation}
...
\end{equation*}
Now we write $z = u/v$ and $\phi(z) = u^\prime / v^\prime$ with $u^\prime = a u + b v$ and $v^\prime = c u + d v$. We need to show that
\begin{equation*}
...
\begin{equation*}
P \cdot \cvec{u}{v} = \cvec{a z + b}{c z + d} = \cvec{u^\prime}{v^\prime}.\qedhere
\end{equation*}
\end{proof}
