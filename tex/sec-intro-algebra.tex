...
\index{Group!action}
Let $\langle G, \cdot, 1 \rangle$ be a group and $S$ be an arbitrary set. A function $G \times S \to S$, $(g, x) \mapsto g x$ is called a \emph{group action} on the set $S$ (we say $G$ \emph{acts on the set} $S$), if
\begin{equation*}
...
The action of a group $G$ on itself induces a monomorphism from $G$ to the group of permutations of the elements of $G$. Hence every group is isomorphic to a permutation group. In particular every finite group is isomorphic to a subgroup of a symmetric group $S_n$ with $n = |G|$.
\end{corollary}
