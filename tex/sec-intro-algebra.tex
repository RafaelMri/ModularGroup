...
\index{Group!action}
Let $\langle G, \cdot, 1 \rangle$ be a group and $S$ be an arbitrary set. A function $G \times S \to S$, $(g, x) \mapsto g x$ is called a \emph{group action} of $G$ on the set $S$ (alternatively we say $G$ \emph{acts on the set} $S$), if
\begin{equation*}
