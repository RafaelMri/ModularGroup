...
If $M \in \GL{\R}$, then the corresponding equivalence class $[M]_{\sim} \in \PGL{\R}$ consists of all matrices $\lambda M$, $\lambda \in \R \setminus \{0\}$. If $\det(M) > 0$ then among these matrices, there is exactly one matrix with determinant 1, namely $M^{\prime} = \reci{\sqrt{\det(M)}} M$. We can therefore identify the equivalence class $[M]_{\sim} \in \PGL{R}$ with $[M^{\prime}]_{\sim} \in \PSL{\R}$. In the other case, when $\det(M) < 0$, we cannot find such a matrix $M^{\prime}$, as the square root of a negative number is not real. Therefore $\PSL{\R}$ is a proper subgroup of $\PGL{\R}$,
\begin{equation*}
\PSL{\R} \lneq \PGL{\R}.
\end{equation*}
