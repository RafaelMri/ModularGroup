...
If $G$ is a nonempty set, a \emph{binary operation} on $G$ is a function $G \times G \to G$. Commonly used notations for the image of $(a,b) \in G \times G$ under a binary operation are $ab$ (product notation), $a + b$ (additive notation), $a \cdot b$, $a \ast b$, etc. In this chapter and also later on we will use the \emph{product notation}, $(a,b) \mapsto ab$, most frequently and we refer to $ab$ as the \emph{product} of $a$ and $b$.
...
\end{enumerate}
then $G$ is called a \emph{group}. If only axioms (\ref{itm_Associativity}) and (\ref{itm_IdentityExists}) are satisfied, then $G$ is called a \emph{monoid}. In both cases, the notation $\langle G,e,\cdot \rangle$ will be used for making the identity element and the involved binary operation explicit.
\end{definition}
