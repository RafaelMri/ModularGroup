...
\begin{theorem}
Let $\mathcal{H}$ and $\EU$ be defined as in (\ref{eqn_UpperHalfplane}) and (\ref{eqn_ExtUpperHalfplane}). The set
\begin{equation}
\tilde{\mathcal{F}} := \setdefsz{\bigg}{z \in \C}{\abs{\Re{z}} < \reci{2} \text{ and } \abs{z} > 1}
\end{equation}
is a fundamental region for the action of $\PSL{\Z}$ on $\EC \setminus \Irrat$. The part of $\tilde{\mathcal{F}}$ lying in the upper halfplane $\mathcal{H}$, \ie the set
\begin{equation}
\mathcal{F} := \tilde{\mathcal{F}} \cap \mathcal{H}
\end{equation}
is fundamental with respect to the action of $\PSL{\Z}$ on $\EU$.
\end{theorem}
\begin{proof}
The second statement that $\mathcal{F} = \tilde{\mathcal{F}} \cap \mathcal{H}$ is fundamental region for $\PSL{\Z}$ acting on $\EU$ is a simple consequence of the first statement. For proving that $\tilde{\mathcal{F}}$ is fundamental,  observe that $\tilde{\mathcal{F}}$ is exactly the set
\begin{equation*}
\tilde{\mathcal{F}} = \mathcal{R} \cap \setdef{z \in \C}{\abs{z} > 1},
\end{equation*}
with $\mathcal{R}$ as in (\ref{eqn_PSL2MinRegion}) -- compare also Figure~\ref{fig_PSL2MinRegion}. Obviously $\tilde{\mathcal{F}}$ is a nonempty open subset of $\mathcal{R}$, which is why two distinct points of $\tilde{\mathcal{F}}$ can be equivalent only by the transformation $T$. However, since $\abs{z} > 1$ implies $\abs{Tz} = \abs{-1/z} < 1$, this is impossible. Therefore $\tilde{\mathcal{F}}$ contains no equivalent distinct points.

It remains to show that every $z = \frac{u}{v} \in \mathcal{S}$ is equivalent to a point of the topological closure $\topcl{\tilde{\mathcal{F}}}$ of $\tilde{\mathcal{F}}$. For this purpose apply the algorithm of Theorem~\ref{thm_SL2FunDomAlg} to the vector $({}^u_v) \in \hat{\mathcal{S}}$ in order to obtain a transformation $B \in \PSL{\Z}$ which maps $z$ to a point of $\topcl{\mathcal{R}}$. It then follows that at least one of the points $Bz$ or $TBz$ lies in $\topcl{\tilde{\mathcal{F}}}$.
\end{proof}

We now wish to obtain a \emph{fundamental set} for the action of $\PSL{\Z}$ on $\EU$. For this purpose we need to consider the boundary of $\mathcal{F}$ which we segment into the following four ``boundary arcs'':
\begin{eqnarray*}
...
\end{eqnarray*}
Clearly these boundary arcs are mapped onto each other by
\begin{equation*}
Ua = b \quad \text{and} \quad Tc = d.
\end{equation*}
To see that no additional transformations other than $U$, $\inv{U}$ and $T$ can establish equivalence of boundary points of $\mathcal{F}$, we just need to consider a finite number of transformations: It follows from Theorem~\ref{thm_SL2FunDomGlobMin}, that it is sufficient to look at transformations $A \in \PSL{\Z}$ with $n(A) \le 3$. 
