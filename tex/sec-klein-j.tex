...
The special importance of $J$ lies in the fact that all modular functions can be represented as rational function in $J$ with complex coefficients:
...
\begin{remark}
Given a modular function $F$, then according to Theorem~\ref{thm_ModFunRatJ}, there is a $f \in \C(z)$ such that $F = f \circ J$. Clearly for any arbitrary Möbius transformation $A \in \PGL{\C}$, we trivially have $f \circ J = (f \circ \inv{A}) \circ (A \circ J)$. Clearly $f \circ \inv{A} \in \C(z)$ and therefore . 
\end{remark}
