...
In order to discuss this one-to-one mapping between $\FunDom$ and $\EC$ under $J$, let us denote the boundary arcs of $\FunDom$ again by $a,b,c$ and $d$, as in Figure~\ref{fig_PSL2FunDom}. Moreover, denote by $e := \setdef{\lambda \ii}{\lambda \ge 1} \cup \{\infty\}$ the arc which splits $\FunDom$ into two symmetric halves, \ie two open connected components $\FunDom_\text{left}$ and $\FunDom_\text{right}$. For the special boundary points $\ii$, $\rho$ and $\infty$ of $\FunDom$ we have
\begin{equation*}
...
\label{fig_KleinJ}
\end{figure}
We now wish to plot the $J$ function using the above idea and the color coding $\col(z)$. However, instead of plotting $J$ directly on the upper half-plane, we again prefer to translate the picture from the upper half-plane to the unit disk using the modified Cayley transform, \ie instead of plotting $\col \circ J$ on the upper half-plane, we plot $\col \circ J \circ \inv{\ModCayley}$ on the unit disk. The result is shown in Figure~\ref{fig_KleinJ}. In order to accentuate the symmetry of $J$ with respect to the transformations of the modular group, additionally the image of the modular tessellation under $\ModCayley$ is overlaid in gray. We can now read off the following properties from this illustration:
\begin{enumerate}
...
\item $J$ has a zero of order 3 at every point equivalent to $\rho$: The points colored in black are precisely those vertices of all modular triangles\footnote{We consider the regions $\ModCayley A \FunDom$, $A \in \PSL{\Z}$ as triangles bounded by circular arcs here.} which lie in the interior of the unit disk. These are exactly the points equivalent to $\rho$. The order of a given zero $z$ can be read off by counting the number of times we go through the set of all color tones while walking once around $z$ along a small circular path which does not enclose any other zero except $z$. Applying this to $\rho$ or any of its equivalent points, this yields an order of 3, as we visit the color tone red (as well as every other color tone) exactly three times.
\end{enumerate}

The special importance of $J$ lies in the fact that all modular functions can be represented as rational function in $J$ with complex coefficients (for the proof of this fact we refer to \Schoeneberg{}, Chapter II, §3):

\begin{theorem}
\label{thm_ModFunRatJ}
The set of all modular functions forms a field which is given by $\C(J)$.
\end{theorem}
%\begin{proof}
...
\begin{remark}
Given a modular function $F$, according to Theorem~\ref{thm_ModFunRatJ} there is a rational function $R\in \C(z)$ such that $F = R \circ J$. For any arbitrary Möbius transformation $A \in \PGL{\C}$, we have $\inv{A} \in \C(z)$ and -- since $\C(z)$ is closed under the operation of function composition -- $R \circ \inv{A} \in \C(z)$. For this reason, $F = (R \circ \inv{A}) \circ (A \circ J)$, is a representation of $F$ as rational function in $A \circ J$. In other words, we have $\C(J) = \C(A \circ J)$ for all Möbius transformations $A$.
\end{remark}
...
\end{tabular}
\caption[Rational functions in $J$]{Examples for rational functions in $J$: The function $z \mapsto \inv{J(z)}$ (top-left) is zero at all rational points (hence the dominant black color) and has a pole of order three at all points equivalent to $\rho$. The function $z \mapsto J(z) - 1$ (top-right) has a zero of order two at all points equivalent to $\ii$. For $z \mapsto J(z)^2$ (bottom-left), the order of the zeros at points equivalent to $\rho$ is six, \ie twice the order of the zeros of the original function $J$. Lastly, composing $J$ with the Cayley transform, that is $z \mapsto \ModCayley \circ J(z)$ (bottom-right) yields a zero (\resp pole) of order 1 at every point $z$ for which $J(z) = \ii$ (\resp $J(z) = -\ii$). Its value at rational points is $\ModCayley \circ J(\infty) = \ModCayley(\infty) = \ii$, explaining the dominant green coloring.}
...
\caption[Klein meets Fibonacci]{The composition of the Klein modular invariant $J$ and the generating function of the Fibonacci sequence $\frac{x}{1 - x - x^2}$, \ie $z \mapsto \frac{J(z)}{1 - J(z) - J(z)^2}$. It has poles of order 1 at points $z$ satisfying $J(z) \in \{-\phi, \reci{\phi}\}$, where $\phi$ denotes the golden ratio, $\phi = \frac{1 + \sqrt{5}}{2}$.}
\label{fig_KleinJFib}
\end{figure}
%Figure~{\ref{fig_FunctionsOfJ}} shows exemplary four different modular functions, plotted in the same fashion as we did before for $J$. Note that 
