...
\begin{remark}
Since danger of confusion is minimal, in the following we will use the same name for a generalized circle and its corresponding Hermitian matrix.
\end{remark}

\begin{theorem}
\label{thm_MoebiusGenCircle}
Let $M \in \Mat{\C}{2}{2}$ be a Hermitian matrix with $\det(M) < 0$ and $P \in \PGL{\C}$. The image of the generalized circle $M$ under the Möbius transformation $\phi$ corresponding to the matrix $P$ is the generalized circle  $\transp{(\inv{P})} \cdot M \cdot \inv{P}$.
\end{theorem}
\begin{proof}
Let us write
\begin{equation*}
...
\end{equation*}
Now we apply (\ref{eqn_HomogenousTransform}) and its Hermit transposed pendant
\begin{equation*}
