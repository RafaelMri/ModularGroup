...
\end{equation}
Note that $D \in \R$ and $B = \conj{C}$ are complex conjugates. From an equation of form (\ref{eqn_GenCircle}) we can read off the center and radius of the corresponding circle by
...
In the case when $A = 0$ equation (\ref{eqn_GenCircle}) reads
\begin{equation*}
\Re{C z} = -\half{D},
\end{equation*}
which defines a line on the complex plane. We see this by considering the simpler equation $\Re{z} = -\half{D}$ first, which obviously defines a line parallel to the imaginary axis through the real point $-\half{D}$. Then we observe, that the multiplication with $C$ just rotates this line clockwise by an angle which is given by $\arg(C)$.
...
\end{equation*}
The matrix in the middle is Hermitian and has a negative determinant, because of the condition $BC - AD > 0$ from above. We can now give an exact definition for generalized circles.
...
\begin{remark}
Since danger of confusion is minimal, in the following we will use the same name for a generalized circle and its corresponding Hermitian matrix (which is determined up to a real scalar factor).
\end{remark}

\begin{theorem}
\label{thm_MoebiusGenCircle}
Let $M \in \Mat{\C}{2}{2}$ be a Hermitian matrix with $\det(M) < 0$ and $P \in \PGL{\C}$. The image of the generalized circle $M$ under the Möbius transformation $\phi$ corresponding to the matrix $P \in \PGL{\C}$ is the generalized circle  $\transp{(\inv{P})} \cdot M \cdot \inv{P}$.
\end{theorem}
\begin{proof}
Let us write
\begin{equation*}
...
\end{equation}
We need to show that for all $z$ satisfying (\ref{eqn_GenCircleMat}), $\phi(z)$ lies on the generalized circle $\transp{(\inv{P})} \cdot M \cdot \inv{P}$, that is 
\begin{equation*}
\rvec{\conj{\phi(z)}}{1} \cdot \transp{(\inv{P})} \cdot M \cdot \inv{P} \cdot \cvec{\phi(z)}{1} = 0.
\end{equation*}
Multiplying with the scalar $c z + d$ and its complex conjugate we obtain
\begin{equation*}
\rvec{\conj{az + b}}{\conj{cz + d}} \cdot \transp{(\inv{P})} \cdot M \cdot \inv{P} \cdot \cvec{az + b}{cz + d} = 0.
\end{equation*}
Now we apply (\ref{eqn_HomogenousTransform}) and its Hermit transposed companion
\begin{equation*}
\rvec{\conj{z}}{1} \cdot \transp{P} \transp{(\inv{P})} \cdot M \cdot \inv{P} \cdot P \cdot \cvec{z}{1} = 0,
\end{equation*}
